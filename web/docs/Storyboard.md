...
- Current custom sandbox widgets include `Mood Tag`, `Shot Card`, `Story Beat`, `Swatch Strip`, `Scene Divider`, `Character Card`, `Location Card`, `Dialogue Card`, and `Camera Move`, each living in its own folder under `web/storyboard/extensions/custom/`.
